...
<head>
    ...
    <title>All TV Shows - MovieApp</title>
...
<header class="w-full sticky top-0 z-50 bg-white/90 dark:bg-black/80 backdrop-blur-md shadow">
    <div class="flex items-center justify-between px-6 py-4">
                    ...
                    @foreach($popularGenres as $genre)
                        <a href="{{ url('/tv-genre/'.$genre['id']) }}"
                           class="block px-3 py-2 rounded hover:bg-[#e50914] hover:text-white transition font-bold whitespace-nowrap
                            {{ request()->is('tv-genre/'.$genre['id'].'*') ? 'text-[#e50914]' : 'text-gray-700 dark:text-gray-200' }}">
                            {{ $genre['name'] }}
                        </a>
                    @endforeach
                </div>
        ...
        <!-- Auth buttons (same as movie page) -->
        @auth
...
<main id="pageContent" class="w-full relative opacity-0 translate-y-4 transition-all duration-700 mt-20">
    <div class="max-w-7xl mx-auto px-8 mt-6 text-center">
        <h1 class="text-4xl md:text-5xl font-bold mb-4">All TV Shows</h1>
        <p class="text-gray-700 dark:text-gray-300">Discover trending and top-rated TV shows worldwide.</p>

        <!-- Search Bar -->
        <form method="GET" action="{{ url('/tv-shows') }}" class="max-w-xl mx-auto mt-8 flex">
            <input type="text" name="search" value="{{ $search }}" placeholder="Search TV shows..."
            ...
            <button type="submit"
                    class="px-6 py-3 bg-[#e50914] text-white rounded-r-lg font-semibold hover:bg-[#b20710] transition">
                Search
            </button>
        </form>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-8 py-8 mt-8">
        @if($tvshows->count() > 0)
                ...
                @foreach($tvshows as $tv)
                    ...
                    <div
                        ...
                        <img src="https://image.tmdb.org/t/p/w500{{ $tv['poster_path'] ?? '' }}"
                             alt="{{ $tv['name'] ?? 'Untitled' }}" class="w-full h-72 object-cover rounded-xl">

                        <div
                            class="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition flex flex-col justify-end p-3 rounded-xl">
                            <h3 class="text-md md:text-lg font-semibold truncate text-white">{{ $tv['name'] ?? 'Untitled' }}</h3>
                            <p class="text-sm text-white mb-2">⭐ {{ $tv['vote_average'] ?? 'N/A' }} / 10</p>
            ...
            <div class="mt-8 flex justify-center">
                {{ $tvshows->links('pagination::tailwind') }}
            </div>
        @else
            <p class="text-gray-600 dark:text-gray-400 text-center">No TV shows found.</p>
        @endif
    </div>
</main>
