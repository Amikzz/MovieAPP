...
<head>
    ...
    <title>All Actors - MovieApp</title>
...
<main id="pageContent" class="w-full relative opacity-0 translate-y-4 transition-all duration-700 mt-20">
    <div class="max-w-7xl mx-auto px-8 mt-6 text-center">
        <h1 class="text-4xl md:text-5xl font-bold mb-4">All Actors</h1>
        <p class="text-gray-700 dark:text-gray-300">Discover popular and trending actors from around the world.</p>

        <!-- Search Bar -->
        <form method="GET" action="{{ url('/actors') }}" class="max-w-xl mx-auto mt-8 flex">
            <input type="text" name="search" value="{{ $search }}" placeholder="Search actors..."
    ...
    <div class="max-w-7xl mx-auto px-8 py-8 mt-8">
                ...
                @foreach($actors as $actor)
                    <div
                        ...
                        <img src="{{ $actor['profile_path']
                             ...
                             alt="{{ $actor['name'] ?? 'Unknown Actor' }}" class="w-full h-72 object-cover rounded-xl">

                        <div
                            class="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition flex flex-col justify-end p-3 rounded-xl">
                            <h3 class="text-md md:text-lg font-semibold truncate text-white">{{ $actor['name'] ?? 'Unknown Actor' }}</h3>
                            <p class="text-sm text-white mb-2">🎭 {{ $actor['known_for'] ?? 'N/A' }}</p>

                            <a href="{{ route('actor.show', $actor['id']) }}"
                               class="flex items-center justify-center gap-2 px-3 py-2 rounded-full bg-blue-600 hover:bg-blue-700 text-white font-semibold shadow hover:scale-105 transition">
        ...
        @else
            <p class="text-gray-600 dark:text-gray-400 text-center">No actors found.</p>
        @endif
    </div>
</main>
